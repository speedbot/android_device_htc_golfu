...
extern "C" int get_number_of_cameras(void)
{
   ...
   void *libcameraHandle = ::dlopen("libcamerapp.so", RTLD_NOW);
   LOGD("HAL_get_number_of_cameras: loading libcamera at %p", libcameraHandle);
   if (!libcameraHandle) {
       LOGE("FATAL ERROR: could not dlopen libcamerapp.so: %s", dlerror());
   } else {
                   ...
                   hw_device_t** device)
{
   ...
   libcameraHandle = ::dlopen("libcamerapp.so", RTLD_NOW);
